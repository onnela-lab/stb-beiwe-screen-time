...
for (DAYS_DIFF_VALUE_MAX in DAYS_DIFF_VALUE_MAX_VEC){
  ...
  mod_df_w <- 
    mod_df %>% 
    filter(daylight_type == "daylight_start") %>%
  ...
  # save fit object
  fit_fname <- paste0("fit_result_lfosr3s_daylight_start_", DAYS_DIFF_VALUE_MAX, ".rds")
